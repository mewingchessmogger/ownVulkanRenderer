...
void initPipeline() {
	...
	dynRenderInfo.setColorAttachmentCount(1)
				 .setPColorAttachmentFormats(&_swapchainFormat);



	vk::GraphicsPipelineCreateInfo createInfo{};
	createInfo.setPNext(dynRenderInfo)			// setting or dynrendering
			  .setFlags(vk::PipelineCreateFlags{})// tweakiung pipelines set to nothing
			  .setStageCount (1)			// tells vulkan how many shader stages hooked up
			  .setPStages(nullptr)
			  ...
			  .setLayout(vk::PipelineLayout{})
			  .setRenderPass(nullptr)
			  .setSubpass(0)
			  .setBasePipelineHandle(vk::Pipeline{})
			  .setBasePipelineIndex(-1);
		
	_graphicsPipeline = _device.createGraphicsPipeline(VK_NULL_HANDLE, createInfo).value;
...
void draw() {
	...
	vk::Semaphore imageReadySemaph = _imageReadySemaphores[currentFrame];
	

	imageIndex = _device.acquireNextImageKHR(_swapchain, 1000000000, imageReadySemaph).value;

	vk::CommandBuffer cmdBuffer = _cmdBuffers[currentFrame];
	...
	transitionImage(curImage, cmdBuffer, vk::ImageLayout::eGeneral, vk::ImageLayout::ePresentSrcKHR);
	...
	subInfo.setWaitSemaphoreInfos(waitInfo).setSignalSemaphoreInfos(signalInfo).setCommandBufferInfos(cmdInfo);
	...
	presInfo.setWaitSemaphores({ renderFinishedSemaph}).setSwapchains(_swapchain).setImageIndices({ imageIndex })	;
